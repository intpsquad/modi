...
~~**결과는 9+ 또는 13+ 가 정상이다**(UGC 때문).~~ → **2026-08-19: 새 설문에서는 `4+` 가 정상이다**(아래 인용 블록). 다만 **낮추려고 사실과 다르게 답하지 말 것** 은 그대로 유효하다 — 등급은 답의 결과일 뿐이다.

> 🔴 **2026-08-19 — 위 "9+ 또는 13+" 는 옛 설문 기준이라 더 이상 맞지 않는다. `4+` 가 정답이다.**
>
> Apple 이 연령 등급 설문을 개편했다(App Store Connect 배너 *"연령 등급 관련 새로운 소셜
> 미디어 질문 검토"* 가 그것이었다). **UGC 는 이제 등급을 올리는 항목이 아니라 「제공 기능」
> 선언일 뿐이고**, 등급은 2~6단계의 **콘텐츠 빈도**(성적 테마·의료·성적 노출·폭력·도박)로만
> 계산된다. 우리는 그 다섯이 전부 "없음" 이라 **`4+`** 가 나온다.
>
> 실제 설문 화면에서 확인했다 — **1단계 「사용자 생성 콘텐츠」= 예** 인 상태로 **7단계
> 「계산된 등급」= 4+** 다. 심사도 그 값으로 통과했다(2026-08-19 출시).
>
> **한때 이것을 "잘못 제출됐다"(이슈 #35)로 보고 Guideline 2.3 위험이라고 적었는데 오진이었다.**
> 위 표의 답(웹 접근 아니요 / UGC 예 / 채팅·소셜 아니요)은 **그대로 유효하다.** 바뀐 것은
> "그 답이면 9+ 이상이 나온다"는 예상뿐이다.
>
> ⚠️ 표에서 유일하게 논쟁 여지가 있는 것은 **「메시지 및 채팅」**이다. 자료 댓글이 있지만
> 설문 설명이 *"사용자가 서로 **직접 대화**"* 라 1:1 대화가 없는 우리는 "아니요"가 맞다 —
> 이 상태로 심사를 통과했다.
>
> **새 설문의 1단계 답(전부 실측 기준)**: 유해 콘텐츠 차단 아니요 · 나이 확인 아니요 ·
> 제한되지 않은 웹 액세스 **아니요** · 사용자 생성 콘텐츠 **예** · 소셜 미디어 아니요 ·
> 13세 미만 소셜 미디어 비활성화 아니요 · 메시지 및 채팅 아니요 · 광고 아니요.
...
2. ~~**연령 등급 — 소셜 미디어 질문** *(2026-08-14 신규 발견)*~~ → ✅ **2026-08-19 확인 완료.**
   배너가 가리키던 것은 Apple 의 **연령 등급 설문 개편**이었다. 7단계 설문을 전부 열어 실측했고
   답이 이미 다 맞다 — 「사용자 생성 콘텐츠」= **예**, 「제한되지 않은 웹 액세스」= **아니요**,
   계산된 등급 **`4+`**. 자세한 것은 위 §A 인용 블록.
   🔴 중간에 이것을 *"연령 등급이 잘못 제출됐다"*(이슈 #35)로 판단했는데 **오진이었고 닫았다** —
   새 설문에서 UGC 는 등급을 올리지 않는다.
