...
> 🔴 **2026-08-16 — 실제 제출은 이 표와 반대로 들어갔다. App Store Connect 에서 직접 확인했다.**
>
> ```
> 연령 등급  4+   (172개 국가 / 브라질 6세 이상 / 대한민국 전체 / 베트남 00+)
> ```
>
> **4+ 는 「사용자 생성 콘텐츠」에 "없음"으로 답해야만 나온다.** 위 표는 "예"라고 적어 뒀는데
> 그렇게 답했으면 9+ 이상이 됐어야 한다. 앱에는 UGC 가 분명히 있다 — 투두·자료·메모를
> 방 멤버와 공유하고, 자료 댓글과 방 이름·목표가 전부 자유 입력이다.
>
> **Guideline 2.3(부정확한 메타데이터)** 위험이고, 원래 알고 있던 **1.2(UGC 조정 수단 부재)**
> 와도 맞물린다.
>
> **지금은 고치지 않는다**(2026-08-16 사용자 확정). `1.0.0 (5)` 가 심사 중이고, 심사 중
> 연령 등급 변경은 제출을 취소시킬 수 있다. **빌드 6 에서 이 표대로 다시 답한다.**
> 추적은 이슈 #35.
...
2. ~~**연령 등급 — 소셜 미디어 질문** *(2026-08-14 신규 발견)*~~ → 🔴 **2026-08-16: 더 큰 문제로
   드러났다.** 배너는 여전히 떠 있고("'앱 정보' 섹션에서 소셜 미디어 기능에 관한 새로운 질문이
   앱에 적용되는지 확인하십시오"), 확인해 보니 **연령 등급 자체가 `4+` 로 잘못 들어가 있다** —
   「사용자 생성 콘텐츠」에 "없음"으로 답해야만 나오는 값인데 앱에는 UGC 가 분명히 있다.
   경위는 위 §A 의 2026-08-16 인용 블록. **빌드 6 에서 §A 표대로 다시 답한다**(이슈 #35).
