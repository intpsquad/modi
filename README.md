...
#### 🔴 옛 프로젝트에서 로컬 파일을 복사해 오면 **옛 카카오 앱 키가 딸려온다** *(2026-08-14 실제 사고)*

로컬 전용 파일은 각자 준비하는 것이라 **옛 작업 PC 에서 그대로 복사**하기 쉬운데, 그러면 `KAKAO_NATIVE_APP_KEY` 가 **지금 쓰는 카카오 앱의 키가 아닐 수 있다.** 실제로 첫 TestFlight 빌드가 그랬다.

증상이 헷갈린다 — **키 자체는 유효하므로 카카오톡 앱이 정상적으로 열리고, 돌아온 뒤에야 실패한다.**

```
카카오톡 열림          ✅  키가 유효하기만 하면 열린다
앱으로 복귀            ✅
"카카오 로그인에 실패"  ❌
서버 로그              (아무것도 없다 — SDK 단계에서 끊겨 요청이 오지 않는다)
```

**번들 ID 를 아무리 등록해도 안 고쳐진다.** 번들 ID 는 *그 키가 속한 카카오 앱* 에 등록되는 것이라, 빌드가 다른 앱의 키를 들고 있으면 짝이 안 맞는다.

**확인 방법**: 카카오 콘솔 → 앱 → **플랫폼 키** → 네이티브 앱 키가 `Local.xcconfig`·`env/prod.json` 의 값과 같은지 본다. ⚠️ **앱 하나에 네이티브 앱 키가 여러 개일 수 있다**(`Default(대표)` + 플랫폼별). **`com.intpsquad.modi` 가 등록된 카드의 키**를 써야 한다. 테스트 앱(`MODI-TEST`)은 또 다른 키를 가지므로 헷갈리지 말 것.

⚠️ **두 파일의 값이 서로 같아야 한다.** 하나는 `Info.plist` 의 URL 스킴(카카오톡에서 돌아오는 통로), 하나는 Dart 의 `KakaoSdk.init` 에 쓰인다. 어긋나면 **카카오톡은 열리는데 돌아오지 못한다.** `Local.xcconfig` 는 xcconfig 문법이라 `=` 앞뒤 공백을 지우면 **빌드는 성공하고 URL 스킴만 조용히 빈다.**
...
## AI 투두 추천 — 검수 노드 (`CRITIQUE_MAX_ATTEMPTS`, 기본 `0` = 끔)

생성된 후보를 LLM 이 한 번 더 검사해 "바로 실행할 수 없는 것"을 반려하는 단계다.
**2026-08-14 부터 꺼진 것이 운영값이다.** 원래 `1`(켬)이 운영이고 `0` 은 시연용 임시였는데 뒤집혔다.

**뒤집은 이유**: 재시도가 없어서(`1`) **검수가 한 배치를 전멸시키면 사용자가 빈 화면을 본다.**
심사 제출 직전 데모 방에서 두 번 눌러 두 번 다 그랬다(`후보 검수: 5개 중 0개 통과`).
근거와 대가는 [`ai/docs/DECISIONS.md`](ai/docs/DECISIONS.md) 의 검수 항목에 있다.

운영 방(자료 46건) 앱 실측:

| 값 | 후보 수 | 응답시간 | 부작용 |
|---|---|---|---|
| `0`(현재 기본) | 7~8개 | 5~9초 | `일정 짜기` 같은 **바로 실행 못 하는 후보**가 8개 중 1~2개 |
| `1`(검수 켬) | 1~3개 | 10~13초 | 한 배치가 전멸하면 **빈 화면** |

다시 켜려면:

```bash
# 서버에서 (ssh modi)
vi /home/ubuntu/maramodi/.env      # CRITIQUE_MAX_ATTEMPTS=1 추가
cd <repo>/deploy && ./deploy.sh    # 컨테이너 재기동
```

🔴 **값은 두 곳에 적혀 있다** — [`ai/src/modi_ai/config.py`](ai/src/modi_ai/config.py) 의 코드 기본값과
[`deploy/docker-compose.app.yml`](deploy/docker-compose.app.yml) 의 `${CRITIQUE_MAX_ATTEMPTS:-0}`.
**compose 가 항상 값을 주입하므로 코드 기본값만 바꾸면 아무 일도 일어나지 않는다.**
2026-08-14 에 실제로 그렇게 갈라졌다 — 배포는 성공했는데 동작은 그대로였고, 로그에
`후보 검수` 줄이 계속 찍히는 것으로만 알아챘다. **바꿀 때는 두 곳을 함께 바꾼다.**
