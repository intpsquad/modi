...
- **배포 트리거(2026-08-15 변경)**: **`main` 브랜치 머지 시 자동 배포**. `dev`는 테스트만 돌고 **배포하지 않는다**. 브랜치 3계층(`main` 운영 / `dev` 통합 / `feat/*`)은 `CONTRIBUTING.md`가 단일 진실.
  - *(2026-07-29 확정 → 2026-08-13 재확인까지는 **`dev` 머지 = 배포**였다. 서버가 한 대라 이 서버가 개발 서버 겸 데모 서버라는 것이 근거였다. 팀 협업 규칙을 저장소에 반영하면서 운영을 한 단계 보호하는 쪽으로 뒤집었다 — 대가는 **`dev`에 머지해도 서버가 안 바뀌는 것**이다. 자기 기능을 실제 서버에서 보려면 `main`까지 가야 한다. 경위는 `specs/OPEN.md`.)*
...
- **무중단 배포 = blue/green (2026-08-13 확정)**: Spring을 `spring-blue`·`spring-green` 두 서비스로 정의하고(`docker-compose.app.yml`의 `x-spring` 앵커 하나로 공유) **평상시 한 색만 띄운다**. 배포는 ① 멈춘 색을 빌드·기동해 healthy 대기 → ② **활성 파일**(`${MARAMODI_STATE_DIR}/active-upstream.conf`, 내용은 `reverse_proxy spring-<색>:8080` 한 줄)을 바꾸고 `caddy reload` → ③ 옛 색을 graceful 정지. ①에서 실패하면 전환하지 않고 옛 색이 계속 서비스한다. `caddy reload`는 진행 중인 연결을 떨구지 않는다.
  ...
  - 🔴 **활성 파일은 저장소 밖에 둔다.** 배포 잡이 서버의 작업 트리를 커밋된 내용으로 덮어쓰므로(2026-08-13 이전엔 서버에서 `git reset --hard`, 지금은 러너에서 rsync) 저장소 안에 있으면 **매 배포마다 활성 색이 커밋된 값으로 되돌아가** 트래픽이 방금 세운 옛 색으로 간다.
